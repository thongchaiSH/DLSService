    ...
    try {
        if(_partName == window.location.pathname) {
            ...
            if(_type==typeSearch){
                ...
                console.log('beofrebur')
                $('#' + Id).blur();
                removeSessionStorageForCriteriaById(_IdAndPath);
                return true;
            }
            else if (_this == typeSelect || _this == typeInput || _this == typeNumber || _this == typeText || _this == typeTextArea) {
                $('#' + Id).val(sessionStorage.getItem(_IdAndPath).split("#")[1]);
                removeSessionStorageForCriteriaById(_IdAndPath);
                return true;
            } else if (_this == typeSelectized || _this == typeTypecombobox) {
...
$.fn.getTagName = function () {
    ...
    return element === undefined || element === null ? element : element.tagName.toLowerCase();
};
