...
FormUtil.checkdate = function (input) {
    ...
    if (input.match(dateformat)) {
        ...
        if (window.session.lang == 'TH' || window.session.lang == 'TH_TH')
            yy -= 543;
...
FormUtil.isDateFormatWithPopOver = function (value, msg) {
    if (FormUtil.isDateFormat(value) == false) {
...
FormUtil.validateFormFunction = function () {
    ...
    $('[validate="true"]').each(function (index) {
        ...
        if($(this).is(":visible")) {
            if (FormUtil.isEmpty($(this).val())) {
                ...
                $(this).attr('data-content', 'กรุณากรอกข้อมูล');
...
FormUtil.validateFormBySelectFunction = function (select) {
    var valueReturn = true;
    $(".popover").remove();
...
FormUtil.validateFormByElementsFunction = function (element) {
    var valueReturn = true;
    $(".popover").remove();
...
FormUtil.validateFormByElementsFunctionMoreThanOneElement = function (element, arrIdNotShow) {
    var valueReturn = true;
    // $(".popover").remove();
    if (arrIdNotShow.length != 0) {
        element.each(function (index) {
            var id = $(this).attr("id");
            if(arrIdNotShow.indexOf(id) < 0){
                if (FormUtil.isEmpty($(this).val())) {
                    $(this).attr('data-toggle', 'popover');
                    $(this).attr('data-content', 'กรุณากรอกข้อมูล');
                    $(this).attr('data-placement', 'bottom');
...
FormUtil.validateFormBySelectFunctionIdNotShow = function (select, notShow) {
    var valueReturn = true;
    $(".popover").remove();
    $('' + select + '').each(function (index) {
        if (FormUtil.isEmpty($(this).val())) {
            $('#' + notShow + '').attr('data-toggle', 'popover');
            $('#' + notShow + '').attr('data-content', 'กรุณากรอกข้อมูล');
            $('#' + notShow + '').attr('data-placement', 'bottom');
            $('#' + notShow + '').popover('show');
            valueReturn = false;
            return valueReturn;
        }
    });
...
FormUtil.inputNumberOnly = function (e) {
    // console.log(e.which);
    if (e.target.value.length == 0 && e.which == 48 ){
        //not Input Zero First
        e.preventDefault();
    }
